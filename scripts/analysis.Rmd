...
prcnt_meeting <- function(x, grp_var1, variables, ...){
  ...
  if(length(grps2keep) > 0) { # remove groups which cannot be calculated by the 
    removed_grps <- distinct(x, !!sym(grp_var1)) %>% pull() # function here
    removed_grps <- setdiff(removed_grps, grps2keep) %>%  # we will also save
...
sum2 <- sum1 %>% select(Veg_type, FH_MossCover, weight)
sum2 <- drop_na(sum2, Veg_type)

out <- prcnt_meeting(x = sum1, grp_var1 = 'Veg_type',  variables = 'AH_NoxPerenForbCover',  
                     pctval = c(70, 80), conf = 80 )



test <- colnames(sum1)
out <- vector(mode = 'list', length = length(test))
for (i in seq(test)){
  out[[i]] <- prcnt_meeting(x = sum1, grp_var1 = 'Veg_type',  variables = test[i],  
                     pctval = c(70, 80), conf = 80 )
}
