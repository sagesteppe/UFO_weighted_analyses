...
prcnt_meeting <- function(x, grp_var1, variables, ...){
  ...
  if(length(grps2keep) > 0) {
    removed_grps <- distinct(x, !!sym(grp_var1)) %>% pull() ; removed_grps <- setdiff(removed_grps, grps2keep)
  ...
  return( list( return_vals, removed_grps) )
...
out <- prcnt_meeting(x = sum2, grp_var1 = 'Veg_type', variables = 'FH_MossCover', #, 'Hgt_NonNoxPerenGrass_Avg'), 
