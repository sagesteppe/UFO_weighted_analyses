...
prcnt_meeting <- function(x, grp_var1, variables, ...){
  ...
  grp_var1_enq <- enquo(grp_var1)
  variables_enq <- enquo(variables)
  
  grps2remove <- x %>% 
    group_by(across({{ grp_var1_enq }})) %>% # the GROUPING IS NOT TAKING !!!!  
    drop_na(!!variables_enq) %>% 
    filter(!!variables_enq > 0) %>%  
    mutate(Records = n()) %>% 
    filter(Records < 2) %>% 
    distinct(!!grp_var1_enq, .keep_all = T) %>% 
    st_drop_geometry() %>% 
    pull(!!grp_var1_enq) 
  
  `%notin%` <- negate(`%in%`)
  
  if(length(grps2remove) > 0) {
   x <- filter(x, !! grp_var1_enq %notin% grps2remove)
   } else{x}
...
sum1 <- ungroup(sum1)
out <- prcnt_meeting(x = sum1, grp_var1 = 'Veg_type', variables = 'AH_NoxPerenForbCover', #, 'Hgt_NonNoxPerenGrass_Avg'), 
